...
function accelerate(accel) {
  return `Автомобіль ${this.brand} ${this.model} прискорився до швидкості ${
    this.speed + accel
  } км/год`;
}
...
const brake = function (br) {
  return ` Автомобіль ${this.brand} ${this.model} зменшив до швидкості ${br} км/год`;
};
...
const accel = 50;
const resultOfaccelerate = accelerate.call(car, accel);
console.log(resultOfaccelerate);

// Використовуємо методи для гальмування та передаємо 20
const br = 30;
const resultOfBrake = brake.call(car, br);
console.log(resultOfBrake);
...
function Truck(
  ...
  Vehicle.call(this, this, brand, model, year, mileage);
...
Truck.prototype.toString = function specific(num) {
...
console.log(myTruck.toString(9000));

// Викликаємо метод tow з вагою більшою за towingCapacity
console.log(myTruck.toString(11000));
...
const conect = car.drive.bind(car);
 ...
 * | batteryCapacity | 100               |
 */
const myElectricCar = new ElectricCar("Tesla ", " Model S", 2020, 10000, 100);
